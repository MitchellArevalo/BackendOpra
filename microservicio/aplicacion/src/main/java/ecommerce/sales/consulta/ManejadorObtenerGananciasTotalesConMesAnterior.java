...
@Component
public class ManejadorObtenerGananciasTotalesConMesAnterior {
    private final DaoSale daoSale;

    public ManejadorObtenerGananciasTotalesConMesAnterior(DaoSale daoSale) {
    ...
    public MesAnteriorConMesActual ejecutar(){
        ...
        for (Sale ventas: ventasMesActual) {
            gananciasMesActual = gananciasMesActual.add(ventas.getProfitVenta());
        }
        BigDecimal gananciasMesPasado = new BigDecimal(0);
        for (Sale ventas: ventasMesPasado) {
            gananciasMesPasado = gananciasMesPasado.add(ventas.getProfitVenta());
        }
